...
SELECT 
		...
		ROUND(SUM(INICIAL+SD1+SD3E-SD2-SD3S-FINAL),2) DIF,
		SUM(QTD)
FROM
(
	SELECT B9_COD,SUM(B9_VINI1) INICIAL,0 SD1,0 SD2,0 SD3E,0 SD3S,0 FINAL, 0 QTD
	FROM SB9010 A
	WHERE	A.D_E_L_E_T_=''
			AND A.B9_DATA = @DTFEC
			AND A.B9_QINI!=0
			--AND (A.B9_QINI!=0 OR A.B9_VINI1!=0)5.92
			AND A.B9_COD BETWEEN @CODINI AND @CODFIM
	GROUP BY B9_COD
	UNION ALL
	SELECT D1_COD,0,SUM(D1_CUSTO) SD1,0,0,0,0,0 QTD
	FROM SD1010 A
	...
	GROUP BY D1_COD
	UNION ALL
	SELECT D2_COD,0,0,SUM(D2_CUSTO1) SD2,0,0,0,0 QTD
	FROM SD2010 A
	...
	GROUP BY D2_COD
	UNION ALL
	SELECT D3_COD,0,0,0,SUM(CASE WHEN D3_TM <'500' THEN D3_CUSTO1 ELSE 0 END) SD3E,SUM(CASE WHEN D3_TM >'499' THEN D3_CUSTO1 ELSE 0 END) SD3S,0,0 QTD
	FROM SD3010 A
	WHERE	A.D_E_L_E_T_=''
	...
	GROUP BY D3_COD
	UNION ALL
	SELECT B9_COD,0,0,0,0,0,SUM(B9_VINI1) FINAL,SUM(B9_QINI) QTD
	FROM SB9010 A
	WHERE	A.D_E_L_E_T_=''
			AND A.B9_DATA = @DTFIM
			--AND A.B9_QINI!=0
